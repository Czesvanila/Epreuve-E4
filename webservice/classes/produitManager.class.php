...
class produitManager {
    ...
    {        
        ...
        if ($nbRows > 0)
        {
            $query = "update `produit` set `libProd`=?, `descProd`=?, `prixProd`=?, `imageProd`=?, where `idProd`=?;";
            $traitement = $this->db->prepare($query);
            $param1=$prod->getLib();
            $traitement->bindparam(1,$param1);
            $param2=$prod->getDesc();
        ...
        {
            $query = "insert into `produit` (`idProd`, `libProd`, `descProd`,`prixProd`,`imageProd`) values (?,?,?,?,?);";
            $traitement = $this->db->prepare($query);
            $param1=$prod->getId();
            $traitement->bindparam(1,$param1);
            $param2=$prod->getLib();
    ...
    public function getList($restriction='WHERE 1')
    {
        ...
        while ($row = $result->fetch())
        {
            ...
            $produit = new produit($row['libProd'],$row['descProd'],$row['prixProd'],$row['imageProd']);            
